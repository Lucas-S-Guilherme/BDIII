# Lucas da Silva Guilherme

#CLINICA ODONTOLOGICA
#drop database BD_Clinica_Odontologica_1;
...
SELECT nome_pac FROM Paciente WHERE (nome_pac LIKE 'Jo%' AND data_nascimento_pac > '1990-12-31');

-- 2. Mostre os funcionários com função "Dentista" e email contendo "gmail".
SELECT nome_fun FROM Funcionario WHERE (funcao_fun = 'Dentista' AND email_fun LIKE '%gmail%');

-- 3. Liste os fornecedores cujo nome fantasia contém "Dental" e CNPJ que contenha o número "12".
SELECT nome_fantasia_for FROM Fornecedor WHERE (nome_fantasia_for LIKE '%Dental%' AND cnpj_for LIKE '%12%');

-- 4. Mostre os procedimentos com valor maior que 200 e nome contendo "Canal".
SELECT nome_pro FROM Procedimento WHERE (valor_pro > 200 AND nome_pro LIKE '%Canal%');

-- 5. Liste os pacientes com data de nascimento entre 1980 e 1990 e que moram em endereços com "Rua".
SELECT nome_pac FROM Paciente WHERE (data_nascimento_pac BETWEEN '1980-01-01' AND '1990-12-31' AND endereco_pac LIKE '%Rua%');

-- 6. Selecione os materiais cujo tipo é "Material" e estoque maior que 100.
SELECT nome_mat FROM Material WHERE (tipo_mat = 'Material' AND estoque_mat > 100);

-- 7. Liste os funcionários com especialidade "Ortodontia" e CPF terminado em "00".
SELECT nome_fun FROM Funcionario WHERE (especialidade_fun = 'Ortodontia' AND cpf_fun LIKE '%00');

-- 8. Mostre os agendamentos para o tipo "Avaliação" em salas com ID maior que 2.
SELECT * FROM Agenda WHERE (tipo_age = 'Avaliação' AND id_sal_fk > 2);
...
SELECT * FROM Despesa WHERE (valor_des BETWEEN 500 AND 3000 AND descricao_des LIKE '%luz%');

-- 11. Selecione os pacientes com CPF contendo "123" e celular começando com "(11)".
SELECT nome_pac FROM Paciente WHERE (cpf_pac LIKE '%123%' AND celular_pac LIKE '(11)%');

-- 12. Liste os procedimentos com tempo maior que 30 minutos e requisitos contendo "Raio-X".
SELECT nome_pro FROM Procedimento WHERE (tempo_pro > '00:30:00' AND requisitos_pro LIKE '%Raio-X%');
...
SELECT nome_mat FROM Material WHERE (descricao_mat LIKE '%resina%' AND valor_mat < 50);

-- 16. Liste os funcionários com data de nascimento entre 1980 e 1995 e função "Recepcionista".
SELECT nome_fun FROM Funcionario WHERE (data_nascimento_fun BETWEEN '1980-01-01' AND '1995-12-31' AND funcao_fun = 'Recepcionista');

-- 17. Mostre os agendamentos com data entre '2024-01-01' e '2024-12-31' e tipo "Cirurgia".
SELECT * FROM Agenda WHERE (data_hora_age BETWEEN '2024-01-01' AND '2024-12-31' AND tipo_age LIKE '%Cirurgia%');

-- 18. Selecione pacientes com nome que contenha "Silva" e email contendo "gmail".
SELECT nome_pac FROM Paciente WHERE (nome_pac LIKE '%Silva%' AND email_pac LIKE '%gmail%');

-- 19. Mostre as consultas pagas com forma de pagamento "cartão" e valor maior que 100.
SELECT * FROM Consulta WHERE (forma_pagamento_con LIKE '%cartão%' AND valor_con > 100);

-- 20. Liste salas com nome contendo "atendimento" e número de cadeiras maior que 2.
SELECT nome_sal FROM Sala WHERE (nome_sal LIKE '%atendimento%' AND numero_cadeiras_procedimento_sal > 2);

-- Exercícios de UPDATE

-- 1. Atualize o valor dos procedimentos com nome contendo "Canal" e tempo maior que 1 hora, aumentando em 10%.
UPDATE Procedimento SET valor_pro = valor_pro * 1.1 WHERE (nome_pro LIKE '%Canal%' AND tempo_pro > '01:00:00');

-- 2. Atualize o status dos recebimentos vencidos (data_vencimento_rec < CURDATE()) e valor maior que 100 para "atrasado".
UPDATE recebimento SET status_rec = 'atrasado' WHERE (data_vencimento_rec < CURDATE() AND valor_rec > 100);

-- 3. Atualize o estoque dos materiais cujo nome contenha "agulha" e tipo seja "Descartável", subtraindo 10 unidades.
UPDATE Material SET estoque_mat = estoque_mat - 10 WHERE (nome_mat LIKE '%agulha%' AND tipo_mat = 'Descartável');

-- 4. Altere o status das caixas com data_fechamento anterior a hoje e saldo final menor que 0 para "fechado com erro".
UPDATE Caixa SET status_cai = 'fechado com erro' WHERE (data_fechamento_cai < CURDATE() AND saldo_final_cai < 0);

-- 5. Atualize o endereço dos pacientes cujo CPF contenha "999" e email contenha "gmail", adicionando " - Atualizado".
UPDATE Paciente SET endereco_pac = CONCAT(endereco_pac, ' - Atualizado') WHERE (cpf_pac LIKE '%999%' AND email_pac LIKE '%gmail%');

-- 6. Atualize o valor das despesas com descrição contendo "água" e valor entre 50 e 100 para 120.
UPDATE Despesa SET valor_des = 120 WHERE (descricao_des LIKE '%água%' AND valor_des BETWEEN 50 AND 100);

-- 7. Mude o nome fantasia dos fornecedores com CNPJ começando com "55" e nome fantasia contendo "Dental" para "Dental Pro".
UPDATE Fornecedor SET nome_fantasia_for = 'Dental Pro' WHERE (cnpj_for LIKE '55%' AND nome_fantasia_for LIKE '%Dental%');

-- 8. Atualize o celular dos funcionários cujo nome contenha "Maria" e função seja "Dentista" para "(00) 00000-0000".
UPDATE Funcionario SET celular_fun = '(00) 00000-0000' WHERE (nome_fun LIKE '%Maria%' AND funcao_fun = 'Dentista');

-- 9. Aumente em 15% o valor dos materiais com valor maior que 100 e estoque menor que 20.
UPDATE Material SET valor_mat = valor_mat * 1.15 WHERE (valor_mat > 100 AND estoque_mat < 20);

-- 10. Atualize a função dos funcionários com CPF terminado em "11" e que trabalham como "Atendente" para "Atendente Sênior".
UPDATE Funcionario SET funcao_fun = 'Atendente Sênior' WHERE (cpf_fun LIKE '%11' AND funcao_fun = 'Atendente');

-- 11. Atualize o tipo das salas que têm "cirurgia" no nome e número de cadeiras > 1 para "Cirúrgica".
UPDATE Sala SET tipo_sal = 'Cirúrgica' WHERE (nome_sal LIKE '%cirurgia%' AND numero_cadeiras_procedimento_sal > 1);

-- 12. Mude a forma de pagamento das consultas com valor > 300 e data entre '2024-01-01' e '2024-06-30' para "Pix".
UPDATE Consulta SET forma_pagamento_con = 'Pix' WHERE (valor_con > 300 AND data_con BETWEEN '2024-01-01' AND '2024-06-30');

-- 13. Atualize a quantidade na tabela Consulta_Material para 0 onde a consulta ocorreu em março e material contém "gaze".
UPDATE Consulta_Material SET quantidade_mat_con = 0 WHERE id_con_fk IN (SELECT id_con FROM Consulta WHERE MONTH(data_con) = 3)
AND id_mat_fk IN (SELECT id_mat FROM Material WHERE nome_mat LIKE '%gaze%');

-- 14. Atualize o status dos pagamentos com data de vencimento no passado e status "pendente" para "vencido".
UPDATE pagamento SET status_pag = 'vencido' WHERE (data_vencimento_pag < CURDATE() AND status_pag = 'pendente');

-- 15. Aumente o valor das consultas do paciente com ID 3 e que ocorreram em 2023 em R$ 50.
UPDATE Consulta SET valor_con = valor_con + 50 WHERE (id_pac_fk = 3 AND YEAR(data_con) = 2023);

-- 16. Atualize a especialidade de funcionários cujo nome contenha "Ana" e função seja "Dentista" para "Ortodontista".
UPDATE Funcionario SET especialidade_fun = 'Ortodontista' WHERE (nome_fun LIKE '%Ana%' AND funcao_fun = 'Dentista');

-- 17. Altere a descrição dos procedimentos cujo nome contenha "clareamento" e valor menor que 200.
UPDATE Procedimento SET descricao_pro = 'Procedimento de clareamento dental básico' WHERE (nome_pro LIKE '%clareamento%' AND valor_pro < 200);

-- 18. Atualize o número de cadeiras das salas tipo "Padrão" com descrição contendo "pequena" para 1.
UPDATE Sala SET numero_cadeiras_procedimento_sal = 1 WHERE (tipo_sal = 'Padrão' AND descricao_sal LIKE '%pequena%');

-- 19. Corrija o nome dos materiais cujo nome contenha "resnina" e tipo seja "Estético" para "resina".
UPDATE Material SET nome_mat = 'resina' WHERE (nome_mat LIKE '%resnina%' AND tipo_mat = 'Estético');

-- 20. Atualize o valor do procedimento "Cirurgia" com tempo > 1h e requisitos contendo "anestesia" para R$ 500.
UPDATE Procedimento SET valor_pro = 500 WHERE (nome_pro LIKE '%Cirurgia%' AND tempo_pro > '01:00:00' AND requisitos_pro LIKE '%anestesia%');

-- Exercícios de DELETE

-- 1. Delete os pacientes com nome contendo "Teste" e CPF terminando em "000".
DELETE FROM Paciente WHERE (nome_pac LIKE '%Teste%' AND cpf_pac LIKE '%000');

-- 2. Exclua os agendamentos com data anterior a 2022 e tipo "Avaliação".
DELETE FROM Agenda WHERE (YEAR(data_hora_age) < 2022 AND tipo_age = 'Avaliação');

-- 3. Delete os fornecedores com CNPJ duplicado e nome fantasia contendo "Genérico".
DELETE FROM Fornecedor WHERE nome_fantasia_for LIKE '%Genérico%' AND cnpj_for IN (SELECT cnpj_for FROM Fornecedor GROUP BY cnpj_for
HAVING COUNT(*) > 1);

-- 4. Remova os materiais com estoque igual a 0 e tipo "Descartável".
DELETE FROM Material WHERE (estoque_mat = 0 AND tipo_mat = 'Descartável');

-- 5. Delete procedimentos com valor menor que 50 e nome contendo "teste".
DELETE FROM Procedimento WHERE (valor_pro < 50 AND nome_pro LIKE '%teste%');

-- 6. Exclua despesas com data anterior a 2020 e valor menor que 100.
DELETE FROM Despesa WHERE (YEAR(data_des) < 2020 AND valor_des < 100);

-- 7. Delete os funcionários com RG vazio e email terminando com "@teste.com".
DELETE FROM Funcionario WHERE (rg_fun = '' OR rg_fun IS NULL) AND email_fun LIKE '%@teste.com';

-- 8. Remova as consultas que ocorreram em feriados (ex: data_con = '2024-12-25') e valor = 0.
DELETE FROM Consulta WHERE (data_con IN ('2024-12-25', '2024-01-01', '2024-04-21') AND valor_con = 0);

-- 9. Delete registros de Consulta_Procedimento onde o procedimento tem valor < 50 e tempo menor que 15min.
DELETE FROM Consulta_Procedimento WHERE id_pro_fk IN (SELECT id_pro FROM Procedimento WHERE (valor_pro < 50 AND tempo_pro < '00:15:00'));

-- 10. Delete caixas com status "inativo" e saldo final < 0.
DELETE FROM Caixa WHERE (status_cai = 'inativo' AND saldo_final_cai < 0);

-- 11. Exclua os pagamentos com status "cancelado" e valor igual a 0.
DELETE FROM pagamento WHERE (status_pag = 'cancelado' AND valor_pag = 0);

-- 12. Delete os recebimentos com data de vencimento antes de 2020 e status "pago".
DELETE FROM recebimento WHERE (YEAR(data_vencimento_rec) < 2020 AND status_rec = 'pago');

-- 13. Exclua registros da tabela Consulta_Material onde o material não existe mais no estoque e quantidade < 1.
DELETE FROM Consulta_Material WHERE quantidade_mat_con < 1 AND id_mat_fk NOT IN (SELECT id_mat FROM Material);

-- 14. Remova compras com valor = 0 e feitas por funcionário com ID nulo.
DELETE FROM Compra WHERE (valor_com = 0 AND id_fun_fk IS NULL);

-- 15. Delete agendamentos em salas do tipo "Emergência" com data anterior a 2021.
DELETE FROM Agenda WHERE id_sal_fk IN (SELECT id_sal FROM Sala WHERE tipo_sal = 'Emergência') AND YEAR(data_hora_age) < 2021;

-- 16. Exclua procedimentos com nome contendo "falso" e descrição vazia.
DELETE FROM Procedimento WHERE (nome_pro LIKE '%falso%' AND (descricao_pro = '' OR descricao_pro IS NULL));

-- 17. Delete materiais cujo nome contenha "expirado" e estoque < 5.
DELETE FROM Material WHERE (nome_mat LIKE '%expirado%' AND estoque_mat < 5);

-- 18. Remova fornecedores com razão social contendo "inativo" e CNPJ inválido (ex: menos de 14 dígitos).
DELETE FROM Fornecedor WHERE (razao_social_for LIKE '%inativo%' AND cnpj_for < 14);

-- 19. Delete despesas relacionadas a fornecedores que já foram removidos.
DELETE FROM Despesa WHERE id_for_fk NOT IN (SELECT id_for FROM Fornecedor);

-- 20. Exclua pacientes com email inválido (ex: sem "@") e nome contendo "excluir".
DELETE FROM Paciente WHERE (email_pac NOT LIKE '%@%' AND nome_pac LIKE '%excluir%');
