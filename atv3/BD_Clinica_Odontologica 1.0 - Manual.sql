...
SELECT razao_social_for FROM Fornecedor WHERE (nome_fantasia_for LIKE '%Dental%' AND cnpj_for LIKE "%12%");
...
UPDATE recebimento SET status_rec = "atrasado" WHERE (data_recebimento_rec < CURDATE() AND valor_rec > 100);
...
UPDATE Caixa SET status_cai = "Fechamento com erro" WHERE (data_fechamento_cai < CURDATE() AND saldo_final_cai < 0);
...
DELETE FROM Agenda WHERE (data_hora_age < "2022-01-01" AND tipo_age = "Avaliação");

--     Delete os fornecedores com CNPJ duplicado e nome fantasia contendo "Genérico".
DELETE FROM Fornecedor WHERE (cnpj_for = );
